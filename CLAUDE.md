...
**The OpenRouter for web search.** One CLI, pluggable providers for `search`, `scrape`, and `crawl`. Pick a provider per operation; swap them without touching call sites.

Module `github.com/rishang/seek`, rooted in `src/`. Go 1.25, `urfave/cli/v3`, `imroc/req/v3`.

## Layout

- `src/main.go` — CLI commands (`search`, `scrape`, `crawl`, `config`), flag parsing, wiring.
...
- `src/cache/` — SQLite result cache (scrape/crawl only; search always hits the provider).
...
- **The abstract type the factory is built on is the `Provider` interface** (`provider/provider.go`),
  ...
  into the capability interfaces it supports — `SearchProvider`, `ScrapeProvider`, `CrawlProvider`.
...
- **Capabilities, not classes.** A provider implements whichever of `SearchProvider` /
  `ScrapeProvider` / `CrawlProvider` it supports; there is no provider base class, only the
...
- **Options flow** CLI → `config.SearchOptions`/`ScrapeOptions` → each provider formats to its own
